...
class Axiom implements Axiom {
    ...
    private checkACL = (ip: string, domain: string) => {
        ...
        for (let i = 0; i < this.acl.length; i++) {
            if (
                (this.acl[i].type === 'special_ranges' && ipAddr && ipAddr.range() !== 'unicast') ||
                (this.acl[i].type === 'ipv4' && ipAddr && ipAddr.kind() === 'ipv4' && ipAddr.match(this.acl[i].match as [ipaddr.IPv4, number])) ||
                (this.acl[i].type === 'ipv6' && ipAddr && ipAddr.kind() === 'ipv6' && ipAddr.match(this.acl[i].match as [ipaddr.IPv6, number])) ||
                (this.acl[i].type === 'domain' && this.checkDomain(domain, this.acl[i].match as string))
...
const axiom = (acl: string | { match: string; action: string }[]) => {
